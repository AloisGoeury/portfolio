...
export class ProjectsService {
  ...
  async listAdmin(): Promise<Project[]> {
    const result = await this.database.query<Project>(projectsSql.listAdmin);
    return result.rows;
  }
  ...
  async create(dto: CreateProjectDto): Promise<Project> {
    try {
      const id = await this.database.transaction(async (client) => {
        const result = await client.query<{ id: string }>(projectsSql.create, [
          dto.title,
          dto.slug,
          dto.summary,
          dto.contentMarkdown ?? '',
  ...
  private async replaceRelations(
    ...
    for (const tag of uniqueTags) {
      const result = await client.query<{ id: string }>(projectsSql.upsertTag, [
        tag.name,
        tag.slug,
      ]);
      await client.query(projectsSql.attachTag, [projectId, result.rows[0].id]);
    }
