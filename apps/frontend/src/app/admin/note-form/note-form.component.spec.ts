...
describe('NoteFormComponent', () => {
  it('suggests a normalized slug from the title', async () => {
    await TestBed.configureTestingModule({
      ...
      providers: [
        provideRouter([]),
        {
          ...
          useValue: { create: vi.fn(), update: vi.fn(), findAdmin: vi.fn() },
        },
        {
    ...
    fixture.componentInstance.form.controls.title.setValue('Étape importante');
  ...
  it('loads and updates an existing note without a linked project', async () => {
    ...
    expect(notesApi.update).toHaveBeenCalledWith(
      'note-id',
      expect.objectContaining({ title: 'Note existante', projectId: null }),
    );
  });
});
