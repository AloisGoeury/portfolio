...
describe('ProjectDetailComponent', () => {
  it('links to the latest GitHub commit', async () => {
          ...
          useValue: {
            findPublished: () =>
              of({
                ...
                tags: [{ id: 'tag-id', name: 'Angular', slug: 'angular' }],
                links: [
                  {
                    id: 'link-id',
                    label: 'Dépôt',
                ...
                lastCommitUrl: 'https://github.com/example/repo/commit/abc',
    ...
    ).find((link) => link.textContent?.includes('Ajoute la synchronisation'));
    expect(commitLink?.href).toBe('https://github.com/example/repo/commit/abc');
  ...
  it('renders a project without optional media or GitHub activity', async () => {
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        {
          provide: ActivatedRoute,
          useValue: { paramMap: of(convertToParamMap({ slug: 'simple' })) },
        },
        {
          provide: ProjectsApiService,
          useValue: {
